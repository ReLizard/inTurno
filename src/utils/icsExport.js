...
export function generateICS(schedule, shifts, startDateStr, endDateStr) {
  ...
  const icsContent = [
    ...
    'CALSCALE:GREGORIAN',
    'METHOD:PUBLISH',
    'X-WR-CALNAME:inTurno - I miei Turni',
...
export function createICSFile(schedule, shifts, startDateStr, endDateStr, calendarName = 'inTurno - I miei Turni') {
  const shiftMap = new Map(shifts.map(s => [s.code, s]));
  const events = [];
  ...
  sortedDates.forEach(dateStr => {
    ...
    const entry = schedule[dateStr];
    if (!entry || !entry.shiftCode || entry.shiftCode === 'R') return;
